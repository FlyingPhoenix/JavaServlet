 ...
 */
@WebFilter(filterName = "OpenAddrecordUser")
public class OpenAddrecordUser implements Filter {
    public void destroy() {
    }
    ...
    public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain) throws ServletException, IOException {
        ...
        if ((login.compareTo("")!=0) && (date.compareTo("")!=0) && (time.compareTo("")!=0)
                && (desc.compareTo("")!=0) && (amount.compareTo("")!=0) &&
                (comment.compareTo("")!=0)) { chain.doFilter(req, resp);}
        else {
            PrintWriter out = resp.getWriter();
            resp.setContentType("text/html");
            out.println("<html><body>");
            out.println("<h1>Incorrect data!</h1>");
            out.println("<form action=\"http://"+url+"/Expenses_war_exploded/AddRecordUser\">");
